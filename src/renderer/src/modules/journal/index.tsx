...
function Stat({ label, value, tone }: { label: string; value: string; tone?: string }): React.JSX.Element {
  return (
    <div className="rounded-lg border border-edge bg-panel p-3">
      ...
      <div className={clsx('num mt-1 text-xl font-bold', tone ?? 'text-text')}>{value}</div>
    </div>
  )
}
...
export default function JournalModule(): React.JSX.Element {
  ...
  const edge = useMemo(() => {
    ...
    return { winRate, totalR, expectancy, byGrade, kzWr: wr(kz), noKzWr: wr(noKz), kzN: kz.length, noKzN: noKz.length }
  }, [closed])
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2 border-b border-edge px-4 py-3">
        ...
        <h1 className="text-[15px] font-semibold text-text">Trade Journal</h1>
        <span className="rounded bg-panel2 px-1.5 py-0.5 text-[10px] text-muted">
          conviction-linked · {trades.length} trades
        </span>
      </div>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-3 gap-4 overflow-y-auto p-4">
        ...
        <div className="col-span-2 space-y-4">
          <form onSubmit={log} className="rounded-lg border border-edge bg-panel p-3">
            <div className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-muted">
              Log a trade (captures live conviction)
            </div>
            ...
            <button
              type="submit"
              disabled={snapping}
              className="mt-3 flex items-center gap-1.5 rounded bg-gold/20 px-3 py-1.5 text-xs font-medium text-gold hover:bg-gold/30 disabled:opacity-50"
            >
          ...
          {open.length > 0 && (
            <div className="rounded-lg border border-edge bg-panel">
              ...
              {open.map((t) => (
                <div key={t.id} className="flex items-center gap-3 border-b border-edge/40 px-3 py-2">
                  <span className={clsx('text-[13px] font-medium', t.side === 'long' ? 'text-up' : 'text-down')}>
                    {t.symbol} {t.side}
                  </span>
                  ...
                  <div className="ml-auto flex gap-1">
                    <button onClick={() => closeTrade(t, 'win')} className="rounded bg-up/15 px-2 py-1 text-[11px] text-up hover:bg-up/25">Win</button>
                    <button onClick={() => closeTrade(t, 'loss')} className="rounded bg-down/15 px-2 py-1 text-[11px] text-down hover:bg-down/25">Loss</button>
                    <button onClick={() => closeTrade(t, 'be')} className="rounded bg-panel2 px-2 py-1 text-[11px] text-muted hover:text-text">BE</button>
                    <button onClick={() => remove(t.id)} className="rounded p-1 text-muted hover:text-down"><Trash2 size={13} /></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="rounded-lg border border-edge bg-panel">
            ...
            {closed.length === 0 && (
              <div className="p-4 text-center text-xs text-muted">
                Closed trades appear here. Log a few and your edge builds on the right.
              </div>
            )}
            {closed.map((t) => (
              <div key={t.id} className="flex items-center gap-3 border-b border-edge/40 px-3 py-2 text-xs">
                <span className={clsx('font-medium', t.side === 'long' ? 'text-up' : 'text-down')}>{t.symbol}</span>
                ...
                <span
                  className={clsx(
                    ...
                    t.status === 'win' ? 'bg-up/15 text-up' : t.status === 'loss' ? 'bg-down/15 text-down' : 'bg-panel2 text-muted'
                  )}
                >
                  {t.status}
                </span>
                <span className={clsx('num w-12 text-right', (t.resultR ?? 0) >= 0 ? 'text-up' : 'text-down')}>
                  {(t.resultR ?? 0) >= 0 ? '+' : ''}
                  {(t.resultR ?? 0).toFixed(1)}R
                </span>
                <button onClick={() => remove(t.id)} className="text-muted hover:text-down"><Trash2 size={12} /></button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <Stat label="Win rate" value={`${edge.winRate.toFixed(0)}%`} tone={edge.winRate >= 50 ? 'text-up' : 'text-down'} />
            <Stat label="Total R" value={`${edge.totalR >= 0 ? '+' : ''}${edge.totalR.toFixed(1)}`} tone={edge.totalR >= 0 ? 'text-up' : 'text-down'} />
            <Stat label="Expectancy" value={`${edge.expectancy >= 0 ? '+' : ''}${edge.expectancy.toFixed(2)}R`} tone={edge.expectancy >= 0 ? 'text-up' : 'text-down'} />
          ...
          <div className="rounded-lg border border-edge bg-panel p-3">
            <div className="mb-2 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-muted">
              <Award size={13} className="text-gold" /> Win rate by grade
            </div>
            {edge.byGrade.map((g) => (
          ...
          <div className="rounded-lg border border-edge bg-panel p-3">
            <div className="mb-2 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-muted">
              <TrendingUp size={13} className="text-gold" /> Killzone edge
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-text">In killzone</span>
              <span className="num text-up">{edge.kzN ? `${edge.kzWr.toFixed(0)}% · ${edge.kzN}` : '—'}</span>
            </div>
            <div className="mt-1 flex justify-between text-xs">
              <span className="text-text">Outside killzone</span>
              <span className="num text-down">{edge.noKzN ? `${edge.noKzWr.toFixed(0)}% · ${edge.noKzN}` : '—'}</span>
            </div>
          </div>
        </div>
      </div>
